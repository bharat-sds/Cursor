(function() {
	...
	function createButton(text, classes) {
		...
		btn.className = classes || 'wpwv-estimate-btn';
		return btn;
	}
	...
	function renderEstimate(container, valuationText, form) {
		if (!container) return;
		container.innerHTML = '';

		var p = document.createElement('p');

		var text1 = document.createTextNode('Estimated valuation for your watch is ' + (valuationText || '') + '. ');
		p.appendChild(text1);

		var text2 = document.createTextNode('To get in touch with our valuation expert ');
		p.appendChild(text2);
		...
		a.addEventListener('click', function(e) {
			e.preventDefault();
			if (!form) return;
			var submitBtn = qs(form, 'button[type="submit"], input[type="submit"]');
	...
	function estimate(form, estimateBtn) {
		var formId = getFormIdFromForm(form);
		...
		var container = qs(form, '#wpwv-valuation-container');
		if (!container) {
			container = document.createElement('div');
			container.id = 'wpwv-valuation-container';
			// Place just above the submit row for visibility
			var submitContainer = qs(form, '.wpforms-submit-container');
			if (submitContainer && submitContainer.parentNode) {
				submitContainer.parentNode.insertBefore(container, submitContainer);
			} else {
		...
		.then(function(json){
			if (!json || !json.success) {
				container.textContent = (json && json.data && json.data.message) ? json.data.message : 'Unable to calculate estimate right now.';
				return;
			}
			...
			renderEstimate(container, json.data.valuation, form);

			hideElement(estimateBtn);
		})
		.catch(function(err){
			console.error('Estimation AJAX error:', err);
			container.textContent = 'Unable to calculate estimate right now.';
		});
	}

	function init() {
		if (!window.WPWV) return;
		...
		var estimateBtn = createButton('Estimate Valuation', 'wpwv-estimate-btn elementor-button elementor-size-sm wpforms-page-button');
